...
function createStairs() {
	...
	stepMesh.position.y = (2*i+1)*verticalStepHeight/2 + i*stepThickness ;	// half of height: put it above ground plane
	stepMesh.position.z = i*(horizontalStepDepth - 2*stepHalfThickness);			// centered at origin
	scene.add( stepMesh );
	...
	stepMesh.position.y = (2*i+1)*stepThickness/2 + (i+1)*verticalStepHeight;
	// Push step forward by half the depth, minus half the vertical step's thickness
	stepMesh.position.z = (2*i+1)*(horizontalStepDepth/2 - stepHalfThickness);
	scene.add( stepMesh );
    }
}
